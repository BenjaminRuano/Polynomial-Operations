...
% Función para evaluar el polinomio de forma anidada (forma Horner)
function valor = evaluar_polinomio_anidado(p, x)
    % Normalizar el polinomio
    p = normalizar(p, length(p));
    
    % Evaluar el polinomio usando la forma anidada (Horner)
    valor = p(1);  % El primer coeficiente
    for i = 2:length(p)
        valor = valor * x + p(i);  % Fórmula anidada de Horner
    end
end
...
% Evaluación de p(x) en x = 2 usando la forma anidada
valor_anidado = evaluar_polinomio_anidado(p, 2);
disp(['Evaluación de p(x) en x=2 (forma anidada): ', num2str(valor_anidado)]);
